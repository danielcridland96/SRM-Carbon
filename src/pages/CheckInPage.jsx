...
function getNowTime() { const n = new Date(); return `${String(n.getHours()).padStart(2,'0')}:${String(n.getMinutes()).padStart(2,'0')}`; }
...
export default function CheckInPage() {
  ...
  // Auto-open the admin modal when returning from Azure AD login with ?openAdmin=1
  useEffect(() => {
    if (searchParams.get('openAdmin') === '1') setShowAdmin(true);
  }, [searchParams]);
  ...
  const calcCarbon = useCallback(async (postcode, mode) => {
    ...
    calcTimer.current = setTimeout(async () => {
      ...
      setCarbon({ co2: distanceKm * EMISSION_FACTORS[mode].factor, distanceKm });
    }, 600);
  }, [office.postcode]);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      setCountdown(prev => { if (prev <= 1) { clearInterval(iv); reset(); return 15; } return prev - 1; });
    }, 1000);
  ...
  async function submit() {
    ...
    try {
      await api.checkin(rec);
    } catch (e) {
      if (import.meta.env.DEV) console.error('Check-in API:', e);
    }
  ...
  function reset() {
    ...
    setVisitDate(getToday()); setArrivalTime(getNowTime());
    ...
    <div className="page-wrapper">
      ...
      {submitted && summary ? (
        <div className="success-screen">
          ...
          <p>Your visit has been logged and the reception team has been notified.</p>
          <div>
          ...
          <p style={{ marginTop: 16, fontSize: 13, opacity: 0.55 }}>Returning to the form in {countdown}s…</p>
        ...
        <div className="card">
          ...
          <div className="field-grid">
            ...
            <div className="field">
              ...
              <input type="date" value={visitDate} max={getToday()} onChange={e => setVisitDate(e.target.value)} />
            </div>
            <div className="field">
          ...
          {carbon && transport && (
            <CarbonResult co2={carbon.co2} distanceKm={carbon.distanceKm} transportLabel={EMISSION_FACTORS[transport].label} />
          )}

          <div className="submit-area">
            <p className="gdpr-note">
              🔒 Data stored securely on Azure with role-based access controls, processed in accordance with GDPR. Retained for 12 months.
            </p>
